...
$sql = "SELECT
            (SELECT COUNT(*) FROM kehadiran WHERE ID_Pelajar = ? AND Status_Kehadiran = 'Lewat') AS lewat_count,
            (SELECT COUNT(*) FROM kehadiran WHERE ID_Pelajar = ? AND Status_Kehadiran = 'Tidak Hadir') AS tidak_hadir_count,
            (SELECT COUNT(*) FROM kehadiran WHERE ID_Pelajar = ? AND Status_Kehadiran = 'Hadir') AS hadir_count,
            (SELECT COUNT(*) FROM gkk) AS gkk_total
        FROM dual";

// Prepare and execute the statement
$stmt = $conn->prepare($sql);
$stmt->bind_param("sss", $muridID, $muridID, $muridID);
$stmt->execute();
...
<!--view more-->
<div class="more-btn">
   <a href="muridLaporan.php" class="inline-option-btn">view all attendance</a>
</div>
</section>
